...
args.save = 'evalimg-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
  ...
  if args.load is not None:
    ckpts = torch.load(args.load)
    print('Load from epoch {}, best_top1_acc {}'.format(ckpts['epoch'],ckpts['best_acc_top1']))
    start_epoch = ckpts['epoch']
    best_acc_top1 = ckpts['best_acc_top1']
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    if args.batch_size/128 == 4:
      warm_rate = [1, 2, 3, 4]
    elif args.batch_size/128 == 2:
      warm_rate = [1, 2]
    if epoch < args.batch_size/128:
      optimizer.param_groups[0]['lr'] = warm_rate[epoch]*args.learning_rate
    else:
      scheduler.step()
    ...
    if epoch % 1 == 0:
      ...
      writer.add_scalar('val_acc_top1', valid_acc_top1, epoch)
      writer.add_scalar('val_acc_top5', valid_acc_top5, epoch)
      writer.add_scalar('val_obj', valid_obj, epoch)
    is_best = False
    if valid_acc_top1 > best_acc_top1:
      best_acc_top1 = valid_acc_top1
      is_best = True
    logging.info('Save best top1 %f', best_acc_top1)
